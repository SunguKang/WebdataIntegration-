...
public class GameBlockingKeyByPlatformYearGenerator extends RecordBlockingKeyGenerator<Game, Attribute> {
	...
	public void generateBlockingKeys(Game record, Processable<Correspondence<Attribute, Matchable>> correspondences,
		...
		String platform;
		if (record.hasValue(Game.PLATFORM)){
		...
		if (record.hasValue(Game.PUBLICATIONDATE)) {
			resultCollector.next(new Pair<>(Integer.toString(record.getPublicationDate().getYear()) + platform, record));
		}
		else {
